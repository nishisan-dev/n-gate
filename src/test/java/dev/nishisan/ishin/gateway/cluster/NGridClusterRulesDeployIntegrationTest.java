...
public class NGridClusterRulesDeployIntegrationTest {
    ...
    // ─── Build da imagem Ishin Gateway ───────────────────────────────────────────

    private static final ImageFromDockerfile ishinImage = new ImageFromDockerfile("ishin-rules-test", false)
    ...
    // ─── Nó 1 (Ishin Gateway com admin API) ─────────────────────────────────────

    @Container
    private static final GenericContainer<?> node1 = new GenericContainer<>(ishinImage)
            .withNetwork(network)
            .withNetworkAliases("ishin-rules-node1")
            .withEnv("SPRING_PROFILES_ACTIVE", "test")
            .withEnv("SERVER_PORT", "9190")
            .withEnv("MANAGEMENT_PORT", "9190")
            .withEnv("ISHIN_CONFIG", "/app/config/adapter-test-cluster-rules.yaml")
            .withEnv("ISHIN_CLUSTER_NODE_ID", "ishin-rules-node1")
            .withEnv("ISHIN_INSTANCE_ID", "rules-test-node-1")
            .withCopyFileToContainer(
    ...
    // ─── Nó 2 (Ishin Gateway com admin API) ─────────────────────────────────────

    @Container
    private static final GenericContainer<?> node2 = new GenericContainer<>(ishinImage)
            .withNetwork(network)
            .withNetworkAliases("ishin-rules-node2")
            .withEnv("SPRING_PROFILES_ACTIVE", "test")
            .withEnv("SERVER_PORT", "9190")
            .withEnv("MANAGEMENT_PORT", "9190")
            .withEnv("ISHIN_CONFIG", "/app/config/adapter-test-cluster-rules.yaml")
            .withEnv("ISHIN_CLUSTER_NODE_ID", "ishin-rules-node2")
            .withEnv("ISHIN_INSTANCE_ID", "rules-test-node-2")
            .withCopyFileToContainer(
                    MountableFile.forClasspathResource("adapter-test-cluster-rules.yaml"),
                    "/app/config/adapter-test-cluster-rules.yaml")
            .withExposedPorts(PROXY_PORT, MGMT_PORT, NGRID_PORT)
